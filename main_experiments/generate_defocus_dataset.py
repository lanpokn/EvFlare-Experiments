...
def filter_light_source_circular(events: Dict, center_x: int = 600, center_y: int = 210, radius: int = 30) -> Dict[str, np.ndarray]:
    """
    圆形光源区域滤波，只保留指定圆形区域内的事件
    ...
    # 计算每个事件到圆心的距离
    distances_squared = (x - center_x) ** 2 + (y - center_y) ** 2
    radius_squared = radius ** 2
    
    # 创建圆形区域内的mask
    circular_mask = distances_squared <= radius_squared
    
    filtered_events = {
        'x': x[circular_mask],
        'y': y[circular_mask],
        'p': events['p'][circular_mask],
        't': events['t'][circular_mask],
    ...
    print(f"Light source circular filtering (center={center_x},{center_y}, r={radius}px):")
...
def main():
    ...
    # 先对noFlare数据应用光源滤波（统一在时间采样前做）
    print("\n=== Applying Light Source Filtering to NoFlare Data ===")
    ...
    for i, sample_start in enumerate(sample_starts):
        ...
        # 保存为DSEC格式
        target_file = os.path.join(target_dir, f"defocus_{int(sample_start*1000)}ms_sample{sample_id}.h5")
        input_file = os.path.join(input_dir, f"defocus_{int(sample_start*1000)}ms_sample{sample_id}.h5")
